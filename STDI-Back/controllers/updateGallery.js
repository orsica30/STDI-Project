...
const postUpdateGallery = async (req, res = response) => {
  ...
  /*Ejecutamos esta función asincrona para eliminar la "gallery" y volverla a crear.
  Con esto nos aseguramos de no estar embasurando la carpeta de la galería del usuario.*/
  const resultDeleteGalleryS3 = await deleteFolderInsideBucket({
  ...
  if (Object.values(files).some((element) => element !== null)) {
    ...
    if (files.galleryImages) {
      ...
      console.log("hasta aquí llegó bien");
      console.log(arrayWithImagesURL);
      ...
      if (updateGallery) {
        res.status(200).json({
          ok: true,
          oldData: galleryExist,
          newData: newGallery,
          msg: "Gallery updated succesfully.",
        });
      } else {
        res.status(404).json({
          ok: false,
          msg: "Gallery can't be updated, please try again.",
        });
      }
    } else {
...
module.exports = { postUpdateGallery };
